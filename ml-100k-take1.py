...
def output_as_text(data, name):
    with open('ml-100k-take1.{}.txt'.format(name), 'w') as f:
    # with open('ml-100k-give10.{}.txt'.format(name), 'w') as f:
        f.write('{} {}\n'.format(ratings.num_users, ratings.num_items))
        for rating in data:
            f.write('{} {} {}\n'.format(rating['user_id'], rating['item_id'], rating['rating']))

output_as_text(ratings.train, 'train')
output_as_text(ratings.val, 'val')
