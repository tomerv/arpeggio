...
def train(
    ...
    for (user_ids, per_user_count, per_user_item_ids, per_user_ratings) in batches:
        ...
        if current_step % FLAGS.evaluate_every == 0:
            ...
            ndcg,mrr = calc_scores.calc_scores(ratings.val, predictions, 10, 'results/ml-100k-take1.bpr.rankings.txt')
            ...
            with tf.Graph().as_default():
                ...
                with sess.as_default():
                    ...
                    for i in range(1):
                        ...
                        U, V, Vb = sess.run(model.get_embedding_mats())
                        np.savetxt('results/ml-100k-take1.bpr.u.txt', U, delimiter=',')
                        np.savetxt('results/ml-100k-take1.bpr.v.txt', V, delimiter=',')
                        np.savetxt('results/ml-100k-take1.bpr.vb.txt', Vb, delimiter=',')
                        predictions = np.matmul(U, np.transpose(V)) + np.transpose(Vb)
